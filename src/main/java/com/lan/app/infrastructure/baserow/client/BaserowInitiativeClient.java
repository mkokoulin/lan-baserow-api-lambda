...
public interface BaserowInitiativeClient {
    ...
    @ClientQueryParam(name = "user_field_names", value = "true")
    BaserowInitiativeRow getByRowId(
        @PathParam("tableId") int tableId,
    ...
    @ClientQueryParam(name = "user_field_names", value = "true")
    @ClientQueryParam(name = "filter__is_visible__boolean", value = "true")
    BaserowListResponse<BaserowInitiativeRow> findAllByExternalId(
        @PathParam("tableId") int tableId,
        @QueryParam("filter__external_id__equal") UUID externalId
    );

    default BaserowInitiativeRow findUniqueByExternalId(int tableId, UUID externalId) {
        var resp = findAllByExternalId(tableId, externalId);

        if (resp.count() == 0 || resp.results().isEmpty()) {
            throw new BaserowNotFoundException("Initiative", externalId);
        }

        if (resp.results().size() > 1) {
            throw new BaserowDataIntegrityException("Initiative", externalId);
        }
