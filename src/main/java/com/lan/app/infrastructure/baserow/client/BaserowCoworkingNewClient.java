...
public interface BaserowCoworkingNewClient {
    ...
    @ClientQueryParam(name = "user_field_names", value = "true")
    @ClientQueryParam(name = "filter__active__equal", value = "true")
    BaserowListResponse<BaserowCoworkingNewRow> getByRowId(
        @PathParam("tableId") int tableId,
    ...
    default BaserowCoworkingNewRow findUniqueByExternalId(int tableId, UUID externalId) {
        ...
        if (resp.count() == 0 || resp.results().isEmpty()) {
            throw new BaserowNotFoundException("Coworking new", externalId);
        }
